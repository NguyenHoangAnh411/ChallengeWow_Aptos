...
export default function PlayerCard({ player, rank, showMedal = false }: PlayerCardProps) {
  ...
  return (
    ...
    >
      <Card className={`glass-morphism rounded-lg p-4 hologram-border relative overflow-hidden group transition-all duration-300 ${
        player.isCurrentUser ? "border-2 border-neon-purple neon-glow-purple" : ""
      }`}>
        ...
        <div className={`absolute top-0 left-0 w-full h-1 transition-all duration-300 ${
          player.status === "answered" ? "bg-green-400" :
          player.status === "thinking" ? "bg-orange-400" :
          player.status === "timeout" ? "bg-red-400" : "bg-gray-600"
        }`}></div>
            ...
            <div className="flex items-center space-x-3">
              ...
              {rank && !showMedal && (
                <div className="w-8 h-8 flex items-center justify-center bg-neon-blue/20 rounded-lg">
                  <span className="text-sm font-bold text-neon-blue">{rank}</span>
                </div>
              )}
              
              <div className="relative">
                <div className={`w-12 h-12 bg-gradient-to-r ${getGradient(rank)} rounded-full flex items-center justify-center animate-glow-pulse`}>
              ...
              <div className="flex-1">
                ...
                {player.walletAddress && (
                  <div className="text-xs text-gray-400 font-mono">
                    {player.walletAddress.substring(0, 6)}...{player.walletAddress.slice(-4)}
                  </div>
                )}
                
                {player.status && (
                  <motion.div 
                    className={`text-xs flex items-center space-x-1 ${getStatusColor(player.status)}`}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    transition={{ duration: 0.3 }}
                  >
                    <div className={`w-2 h-2 rounded-full ${
                      player.status === "answered" ? "bg-green-400" :
                      player.status === "thinking" ? "bg-orange-400 animate-pulse" :
                      player.status === "timeout" ? "bg-red-400" : "bg-gray-400"
                    }`}></div>
            ...
            <div className="text-right space-y-1">
              ...
              {player.responseTime !== undefined && (
                <div className="text-xs text-gray-400 flex items-center space-x-1">
                  <span>⏱</span>
                  <span>{player.responseTime === -1 ? "---" : `${player.responseTime}s`}</span>
                </div>
              )}
